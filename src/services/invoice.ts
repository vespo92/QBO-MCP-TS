...
export class InvoiceService {
  ...
  public async createInvoice(params: any): Promise<QBOInvoice> {
    try {
      ...
      const created = await queueService.add(() => 
        this.api.post<QBOInvoice>('/invoice', invoice)
      );
  ...
  public async updateInvoice(invoiceId: string, updates: Partial<QBOInvoice>): Promise<QBOInvoice> {
    try {
      ...
      const result = await queueService.add(() =>
        this.api.post<QBOInvoice>('/invoice', updated)
      );
  ...
  public async getInvoicePDF(invoiceId: string): Promise<Buffer> {
    try {
      return await queueService.add(() =>
        this.api.downloadPDF('Invoice', invoiceId)
      );
    } catch (error) {
